...
// If your nominal sample rate is 100 Hz:
static constexpr float SAMPLE_RATE_HZ = 110.0f;
static constexpr float Ts = 1.0f / SAMPLE_RATE_HZ;

// Tune if needed; defaults are reasonable
FPA::Params fpaParams;
// fpaParams.predict_rest = false; // if you don't want predicted "REST"
FPA::DetectorRT fpa_left(Ts, fpaParams);
// FPA::DetectorRT fpa_right(Ts, fpaParams); // if you have a second IMU
...
// Helper: print any FPA events that were emitted
static void drainFpaEvents(const char* foot, FPA::DetectorRT& fpa) {
  FPA::Event ev;
  while (fpa.available() && fpa.pop(ev)) {
    const char* tag = "";
    switch (ev.type) {
      case FPA::EventType::FullContact:   tag = "FC"; break;
      case FPA::EventType::HeelRise:      tag = "HR"; break;
      case FPA::EventType::Rest:          tag = "REST"; break;
      case FPA::EventType::RestPredicted: tag = "REST_PRED"; break;
    }
    Serial.printf("EVT,%s,%s,%.3f\n", foot, tag, ev.t * 1000.0f);
  }
}

// Helper: drain drift-corrected trajectories (printed when a stride closes)
static void drainFpaTraj(const char* foot, FPA::DetectorRT& fpa) {
  FPA::TrajSample s;
  while (fpa.trajAvailable() && fpa.popTraj(s)) {
    // time [ms], then v_raw (x,y,z), v_df (x,y,z), drift (x,y,z), pos (x,y,z)
    Serial.printf("TRJ,%s,%.3f,"
                  "%.6f,%.6f,%.6f,"
                  "%.6f,%.6f,%.6f,"
                  "%.6f,%.6f,%.6f,"
                  "%.6f,%.6f,%.6f\n",
                  foot, s.t * 1000.0f,
                  s.v_raw.x, s.v_raw.y, s.v_raw.z,
                  s.v_df.x,  s.v_df.y,  s.v_df.z,
                  s.drift.x, s.drift.y, s.drift.z,
                  s.p.x,     s.p.y,     s.p.z);
  }
}
...
void setup() {
  ...
  // Pass 0 for mag sample time to disable magnetometer
  initVqf(GYR_DT, ACC_DT, 0.0f);
...
if ((haveNewGyr || haveNewAcc) && haveGyrSample && haveAccSample) {
  ...
  float t_sec = 0.001f * t_ms;

  // Get VQF quaternion (6D) and feed causal FPA
  // Print your raw CSV (acc,gyr)
  printCsvSample(t_ms);  
